...
/**
 * 프로그래머스
 * - 코딩테스트 연습 - PCCP기출문제 - 이웃한 칸 (https://school.programmers.co.kr/learn/courses/30/lessons/250125)
 *
 * 문제 설명
...
public class Programmers250125 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution(board1, h1, w1));
    ...
    public static int solution(String[][] board, int h, int w) {
        ...
        for (int i = 0; i < 4; i++) {
            int nx = dx[i] + w; // 다음 x 좌표
            int ny = dy[i] + h; // 다음 y 좌표

            if (nx >= 0 && ny >= 0 && nx < board.length && ny < board.length) {
                final String color = board[ny][nx];
                if (color.equals(target)) answer++;
            }
        }
